...
class RPGame:
    ...
    def startGame(self):
        ...
        while (
            self.isAlive == True and player.currentHP > 0
        ):  # While game is still running and player isn't dead
    ...
    def characterChoice(self, player):  # Allows user to pick a class
        ...
        while self.isAlive == True:  # While user hasn't decided to exit
            ...
            try:  # Checks if user input is an int
                ...
                print(
                    color.Color.RED
                    + "The only valid inputs are: 1, 2, 3, and 0!"
                    + color.Color.END
                )
                sleep(1.5)
            ...
            if userChoice == 1:  # User might pick to be a peasant
                ...
                print(
                    color.Color.RED
                    + "Due to years of backbreaking work, Caldria's peasants are known to have a high tolerance for even the most grueling of tasks.\n\n"
                    + color.Color.END
                )
                classChoice = input(
                    color.Color.DARKCYAN
                ...
                try:
                    ...
                    print(
                        color.Color.RED
                        + "The only valid inputs are 1 and 0!"
                        + color.Color.END
                    )
                    sleep(1.5)
                ...
                print(
                    color.Color.BLUE
                    + "Born to a strong house with servants aplenty, the Caldrian noblemen are considered chivalrous and known to be healthy.\n\n"
                    + color.Color.END
                )
                classChoice = input(
                    color.Color.DARKCYAN
                ...
                print(
                    color.Color.PURPLE
                    + "Caldria's royalty are renown for their short temper, it's said that the magical books they looted from surrounding nations have essentially changed them.\n\n"
                    + color.Color.END
                )
                classChoice = input(
                    color.Color.DARKCYAN
                ...
                print(
                    color.Color.RED + "Invalid selection, try again." + color.Color.END
                )
    ...
    def adventure(self, player):  # Main game function
        ...
        while self.isAlive == True:
            ...
            if userChoice == 1:  # Explore
                ...
                while (
                    userChoice != 0 or player.isAlive == True
                ):  # While user hasn't chosen to exit or they're still
                    ...
                    if encounter == 1:  # Encounter stranger
                        ...
                        print(
                            "Ah, hello my friend. Care to buy something? I have many fine wares for you today."
                        )
                        sleep(1.5)
                ...
                if playerHP >= playerTotalHP * 0.75:  # Top 75% of hp
                    print(color.Color.DARKCYAN + "I'll be fine" + color.Color.END)
                    sleep(1)
                elif (
                    playerHP >= playerTotalHP * 0.50 and playerHP < playerTotalHP * 0.75
                ):  # Between 50 - 75%
                    print(
                        color.Color.DARKCYAN
                        + "Some cuts and bruises, I'm otherwise okay"
                        + color.Color.END
                    )
                    sleep(1)
                elif (
                    playerHP >= playerTotalHP * 0.25 and playerHP < playerTotalHP * 0.50
                ):  # Between 25 - 50%
                    print(
                        color.Color.DARKCYAN
                    ...
                    playerHP >= playerTotalHP * 0.1 and playerHP < playerTotalHP * 0.25
                ):  # Between 1-25%
                    print(
                        color.Color.DARKCYAN
                        + "Need healing, I might not make it"
                        + color.Color.END
                    )
                    sleep(1)
                else:
    ...
    def combat(self, player):
        ...
        while (
            player.currentHP > 0 and opponent.currentHP > 0
        ):  # While player and opponent are still in the game
            ...
            try:
                ...
                print(
                    color.Color.RED
                    + "The only valid inputs are: 1, 2, 3, 4, and 0!"
                    + color.Color.END
                )
                sleep(1.5)
        ...
        if (
            player.isAlive() == True and opponent.isAlive() == False
        ):  # Player survived: rewards
            # TODO: Help me work
    ...
    def displayEnemyStats(self, Enemy):  # Show enemy stats
        print(
            color.Color.DARKCYAN
            + f"{Enemy.className} - HP: {Enemy.getCurrentHP()}/{Enemy.getMaxHP()}"
        )

    def displayCombatStats(
        self, player, mana, stamina
    ):  # Keep track of current combat stats
        print(
            color.Color.DARKCYAN
            + f"{player.className} - HP: {player.getCurrentHP()}/{player.getMaxHP()} | MP: {mana}/{player.getMana()} | EN: {stamina}/{player.getStam()}\n"
            + color.Color.END
        )
    ...
    def displayStats(self, player):  # For most menus
        print(
            color.Color.DARKCYAN
            + f"{player.className} - HP: {player.getCurrentHP()}/{player.getMaxHP()} | MP: {player.getMana()} | XP: {player.getXP()}\n "
            + color.Color.END
        )

    def displayFullStats(
        self, player
    ):  # For when user wants/needs to see their total stats
        print(
            color.Color.DARKCYAN
            + f"{player.className} - HP: {player.getCurrentHP()}/{player.getMaxHP()} | MP: {player.getMana()}\nGP: {player.getGP()} | XP: {player.getXP()}\n"
            + color.Color.END
        )
