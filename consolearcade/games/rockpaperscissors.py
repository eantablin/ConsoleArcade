...
class RockPaperScissors():
	...
	def runGame(self):
		...
		while userChoice != 0: # While the user does not want to quit to game menu
			userChoice = input(color.Color.BLUE + "Play Rock Paper Scissors!\n1. Play\n0. Exit " + color.Color.END)

			try:
				userChoice = int(userChoice)
				isInt = True
			except ValueError:
				print(color.Color.RED + "Invalid selection, try a number instead!" + color.Color.END)
				print(color.Color.RED + "The only valid selections are 1 and 0!" + color.Color.END)
				time.sleep(1.5)
